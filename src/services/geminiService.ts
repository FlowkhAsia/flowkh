...
export const fetchDetailPageData = async (id: number, media_type: 'movie' | 'tv'): Promise<{ details: MovieDetail, cast: Actor[], similar: Movie[] }> => {
  ...
  // For TV shows, we use 'aggregate_credits' to get a more complete cast list across all seasons.
  // For Movies, 'credits' is standard.
  const creditsKey = media_type === 'tv' ? 'aggregate_credits' : 'credits';
  const url = `${TMDB_BASE_URL}/${media_type}/${id}?language=en-US&append_to_response=${creditsKey},similar,videos,images,external_ids`;
  ...
  // Determine source for credits based on media type
  const creditsSource = (media_type === 'tv' && data.aggregate_credits) ? data.aggregate_credits : data.credits;

  const cast: Actor[] = (creditsSource?.cast || [])
      .filter(Boolean)
      .map((actor: any): Actor => {
          let character = actor.character;
          // For aggregate_credits (TV), character info is in the 'roles' array.
          ...
          return {
            id: actor.id,
            name: actor.name,
            character: character || '', // Fallback to empty string if undefined
